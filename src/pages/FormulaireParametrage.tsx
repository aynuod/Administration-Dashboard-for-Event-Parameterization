...
const FormulaireParametrage: React.FC = () => {
  ...
  const [typologiesCible, setTypologiesCible] = useState([]);
  const [typologieCible, setTypologieCible] = useState("");
  const [produitsCible, setProduitsCible] = useState([]);
      ...
      axios
        ...
        .then((response) => {
          ...
          axios
            ...
            .catch((error) => {
              console.error(
                "Erreur lors de la récupération des conditions :",
                error
              );
  ...
  useEffect(() => {
    axios.get(api + "products/domains").then((response) => {
      setTypologiesCible(response.data);
      console.log("Typologies Cible :", response.data);
    });
  }, []);

  const handleTypologieCibleChange = (
    event: React.ChangeEvent<HTMLSelectElement>
  ) => {
    setTypologieCible(event.target.value);
    axios.get(api + `products/product/${typologieCible}`).then((response) => {
      setProduitsCible(response.data);
      console.log("Produits Cible :", response.data);
    });
  };
                  ...
                  <form className="space-y-4" onSubmit={handleAddCondition}>
                    <div className="flex flex-col space-y-2">
                      ...
                      >
                        ...
                        <option value="ModeDeTransaction">
                          Mode de Transaction
                        </option>
                        <option value="Objectif">Objectif</option>
                        <option value="CatégorieClient">
                          Catégorie Client
                        </option>
                      </select>
                    </div>
